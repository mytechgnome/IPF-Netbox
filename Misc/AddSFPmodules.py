...
def infer_interface_type_from_name(name: str) -> Optional[str]:
    ...
    if n.startswith("hundredgige") or n.startswith("100g"):
...
def transform_device_type(doc: CommentedMap) -> Tuple[bool, int, List[str]]:
    ...
    for i in interfaces if isinstance(interfaces, list) else []:
    ...
    for iname, iface in iface_by_name.items():
        ...
        if not is_pluggable(itype):
            continue  # not an SFP-family slot; leave alone
        ...
        if not bay:
            bay = CommentedMap()
            bay["name"] = iname
            bay["position"] = iname  # full path for {module} resolution
            bay["label"] = lbl
    ...
    # 2) From bays -> ensure matching interfaces
...
def main():
    ap = argparse.ArgumentParser(
        description="Ensure device-type SFP bays/interfaces coexist; adapt SFP module types for adopt_components."
    )
    ...
    for src in src_root.rglob("*"):
        ...
        in_device = "device-types" in parts
        in_module = "module-types" in parts
